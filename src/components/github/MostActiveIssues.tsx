...
export default function MostActiveIssues() {
  const { mostActiveIssues, isLoading } = useMostActiveIssues("polkadot");
  const [showAll, setShowAll] = useState(false);

  if (isLoading) return <CardLoader />;
  if (!mostActiveIssues) return;

  const displayedIssues = showAll
    ? mostActiveIssues
    : mostActiveIssues.slice(0, 5);
  ...
  return (
    <div className="border-2 border-indigo-300 rounded-lg py-12">
      <h1 className="ml-12 mb-8">Most Active Issues</h1>
                ...
                <div className="flex items-center text-xs sm:text-sm">
                  {issue.closed ? (
                    <span className="mr-2 px-2 py-1 bg-orange-200 border-2 border-orange-400 text-orange-700 rounded-md text-xs">
                      Closed {formatDistanceToNow(issue.closed_at)}
                    </span>
                  ) : (
